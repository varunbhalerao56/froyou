...
/// Samples the average color of a thin strip at the top or bottom of an image.
///
/// Used to pick the glow colors that the Home backdrop dissolves into, and the
/// background color the rest of the app is themed around.
///
/// Always hand this a [ResizeImage]-wrapped provider. [ui.Image.toByteData]
/// materializes the whole bitmap as RGBA, so a full-resolution phone photo is
/// a ~48MB allocation.
Future<Color> sampleEdgeColor(
  ImageProvider imageProvider, {
  required bool top,
  double stripFraction = 0.08,
  ...
  listener = ImageStreamListener(
    ...
    // Without an error handler and a timeout, a stale or corrupt image path
    // leaves this future pending forever. The path comes from preferences and
    // can outlive the file it points at, and this runs on the boot path — so
    // the failure mode is a frozen splash screen with no exception anywhere.
  ...
  try {
    uiImage = await completer.future.timeout(timeout);
  } finally {
  ...
  final stripHeight = (height * stripFraction).clamp(1, height).toInt();

  final rowStart = top ? 0 : height - stripHeight;
  final rowEnd = top ? stripHeight : height;
...
/// for the accent derived from a photo: a vibrant blue should stay recognizably
/// that blue on a beige background, not collapse into a beige tint.
///
...
Color contrastify(
  ...
  for (final goDarker in darkerIsBetter
      ? const [true, false]
      : const [false, true]) {
    for (double step = 0.02; step <= 1.0; step += 0.02) {
      final lightness = goDarker
          ? hsl.lightness - step
          : hsl.lightness + step;
...
/// they keep a consistent elevation hierarchy whatever photo the theme came
/// from.
